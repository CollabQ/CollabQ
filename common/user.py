...
def get_user_from_request(request):
  user = get_user_from_cookie_or_legacy_auth(request)
...
def get_user_from_cookie_or_legacy_auth(request):
  """attempt to get a logged in user based on the request

  most likely from a cookie
  """
  if 'HTTP_AUTHORIZATION' in request.META:
    username, password = auth_basic_get_credentials(request.META['HTTP_AUTHORIZATION'])
    return lookup_user_by_login(username, password)
...
def auth_basic_get_credentials(authentication):
