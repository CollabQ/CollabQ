...
def actor_url(nick, actor_type, path='', request=None, mobile=False):
  ...
  if (settings.WILDCARD_USER_SUBDOMAINS_ENABLED 
    ...
    if(actor_type == "user"):
      return 'http://%s/%s%s' % (settings.DOMAIN,
                                  nick,
                                  path)
...
class Actor(DeletedMarkerModel):
  ...
  # XXX: Added to have a editable userid
  alias_popular = models.StringProperty()
  ...
  type = models.StringProperty()
  channel_type = models.StringProperty()
  enabled = models.BooleanProperty(default=True)
  ...
  def url(self, path="", request=None, mobile=False):
    ...
    NOTE: if appending a path, it should start with '/'
    """
    if self.alias_popular is not None:
      return actor_url(_get_actor_urlnick_from_nick(self.alias_popular),
               self.type,
               path=path,
               request=request,
               mobile=mobile)
  ...
  # Added to work with API twitter wrapper
  def fullname(self):
    return "%s %s" % (self.extra.get('given_name'), self.extra.get('family_name'))

  def protected_bool(self):
    return self.privacy and True
...
class OAuthConsumer(CachingModel):
  ...
  # XXX (josem@prosoftpeople.com):
  #     This attributes where bring from twitter desing to work
  #     with the same oauth flow
  callback_url = models.StringProperty() # A valid url to call back
  access_type = models.IntegerProperty() # read_write: 1, read_only:0
...
class Relation(CachingModel):
  owner = models.StringProperty()     # ref - actor nick (or parent tag)
  relation = models.StringProperty()  # what type of relationship this is
  target = models.StringProperty()    # ref - actor nick (or son tag)
...
class ChannelTag(CachingModel):
  """
  this represent a tag for Channel in particular, but it can work with
  any Stream
  """
  term          = models.StringProperty()        # The display tag name
  taxonomy      = models.StringProperty()        # The tag type
  path          = models.StringProperty()        # The path to the current

  key_template = '%(path)s/%(term)s'

class HashTagRelation(CachingModel):
  term = models.StringProperty()
  uuid = models.StringProperty()
  
  created_at = properties.DateTimeProperty(auto_now_add=True)

  key_template = '%(term)s/%(uuid)s'

class ExternalProfile(CachingModel):
  nick = models.StringProperty()
  service = models.StringProperty()
  username = models.StringProperty()
  id = models.StringProperty()
  remote_url = models.StringProperty()

class Tag(DeletedMarkerModel):
